...
$(function() {
  var current_locale = $('#current_locale').val(),
      ...
      approve = function ($textarea, approve) {
        ...
        api('approve', data, function() { $textarea.busy('hide'); });
      ...
      deleteTranslation = function ($textarea, approve) {
        ...
        var data = { id: $textarea.siblings('input[name="id"]').val() };
  ...
  function search() {
    ...
    isLongEnough && api('search', { query: query}, onData);
  ...
  function setEvents($container) {
    var saveChangesDebounce = (function() {
      var timeout;

      return function(elem) {
        timeout && clearTimeout(timeout);
        timeout = setTimeout(function() {
          saveChanges.call(elem);
        }, 1000);
      };
    })();
    ...
    $container.find('button.copy').click(function(event) {
      ...
      var original = $button.siblings('.original').children('span').text();
    ...
    function saveChanges() {
      ...
      if (old_value !== new_value) {
        ...
        api('translate', data, function() { $textarea.busy('hide'); });
  ...
  function api(route, data, callback) {
    ...
    function onError() {
      alert('We lost the conection, sorry about that. Please login again.');
