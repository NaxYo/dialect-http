...
module.exports = function (DIALECT_HTTP) {
  ...
  app.get('/:locale/all/:page?', authenticate, function (req, res) {
    var funk = require('funk')(),
        ...
        query = {locale: locale},
        authorized_locales = app.locals.authorized_locales(req, res),
    ...
    if (authorized_locales.some(check_locale)) {
      ...
      funk.parallel(function () {
        ...
        res.render('locale', {
          ...
          paginator: paginator.render({url: '/' + req.params.locale + '/%N', page: req.params.page}),
  ...
  app.post('/:locale/approve', authenticate, function (req, res) {
    if (app.locals.can_approve(req, res)) {
      dialect.store.collection.update(
        {_id: ObjectID.createFromHexString(req.body.id)},
