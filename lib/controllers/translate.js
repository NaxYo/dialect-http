...
module.exports = function (DIALECT_HTTP) {
  ...
  app.get('/set-filter/:filter', authenticate, function(req, res) {
    // Create filter function to get the search content from config file

    req.session.filter = req.params.filter;

    for(var key in filters) {
      if(filters[key].id == req.session.filter) {
        var queryParam = filters[key].search_term;
      }
    }

    console.log('id :', req.session.filter,'search params :', queryParam);

    req.session.addFiltersToQuery = function() {
      var test = 'dududu';
  ...
  app.get('/:locale/:type(all|ok|pending|missing)/:page?', authenticate, function(req, res) {
    ...
    if(res.locals.authorized_locales.some(check_locale)) {
      ...
      req.session.addFiltersToQuery && req.session.addFiltersToQuery();
      console.log(test)
